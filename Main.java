...
public class Main {
	...
	public static void getData() { 
		try { 
			...
			while (rs.next()) { 
				...
				System.out.println("Datum = " + datum + " mit Tankstelle: " + rs.getInt("tankenr") + " und Wert: " + rs.getDouble("value"));
			}     
			//Zuweisung der Daten an die jeweilige richtige Stelle im Array
			for(int i = 0; i <= 26692; i++){
				datum[i] = rs.getInt("datum");
				value[i] = (long) rs.getDouble("value");
				tankenr[i] = rs.getInt("tankenr");
				if(datum[i] - datum[i-1] >= 86400){
					counterTage++;
				}
			}     
	...
	public static int anzahlTanknr() throws SQLException{
		...
		s = "SELECT COUNT(tankenr) FROM (SELECT tankenr FROM sprit_data GROUP BY tankenr)";
		stmt = (Statement) connection.createStatement();
		rs = ((java.sql.Statement) stmt).executeQuery(s);
			
		while(rs.next()){
			a = rs.getInt("count(tankenr)");
			counter++;
		}
		return a;
	}
	...
	public static void main(String[] args) throws ParseException, SQLException, IOException {
		...
		getData();
		anzahlTanknr();
		avaragePreis("2017-03-23 11:35:00,000");
		makePlot();
	}
}
